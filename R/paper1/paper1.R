...
for (celltype in unique(sobj1@meta.data$Cell_Type)) {
  tryCatch({
    perform_DGE_two_days(sobj1, genes=relevant_chromatin_genes, celltype=celltype, day1='E18', day2='P1')
    perform_DGE_two_days(sobj1, genes=relevant_chromatin_genes, celltype=celltype, day1='P1', day2='P4')
    perform_DGE_two_days(sobj1, genes=relevant_chromatin_genes, celltype=celltype, day1='E18', day2='P4')
  }, error = function(e) {
    # Code to run in case of an error
    cat("An error occurred:", e$message, "\n")
  })
}
# for days E18, P1, P4, and reference celltypes
celltypes1 = c('Astrocytes', 'Microglia', 'Oligodendrocytes', 'Interneurons')
celltypes2 = c('Migrating neurons', 'Interneurons', 'Microglia', 'Oligodendrocytes')
for (day in days) {
  for (celltype1 in celltypes1) {
    for (celltype2 in celltypes2) {
      tryCatch({
        if (celltype1 != celltype2) {
          perform_DGE_two_celltypes(sobj1, genes=relevant_chromatin_genes, day=day, celltype1=celltype1, celltype2=celltype2)
        }
      }, error = function(e) {
        # Code to run in case of an error
        cat("An error occurred:", e$message, "\n")
